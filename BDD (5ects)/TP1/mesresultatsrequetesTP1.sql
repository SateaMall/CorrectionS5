...
SELECT LIBELLE, DATESERVICE FROM SERVICE INNER JOIN TRAVAIL ON IDSERV = IDSERVICE WHERE IDVOIS = (SELECT IDVOISIN FROM VOISIN WHERE NOM = 'DURAND' AND PRENOM = 'PAUL');
...
SELECT NOM,PRENOM,LIBELLE,PRIX FROM TRAVAIL INNER JOIN VOISIN ON IDVOISIN=IDVOIS INNER JOIN SERVICE ON IDSERVICE=IDSERV WHERE PRIX >= (SELECT AVG(PRIX) FROM TRAVAIL);
...
SELECT LIBELLE FROM SERVICE WHERE IDSERVICE  NOT IN (SELECT DISTINCT IDSERVICE FROM SERVICE INNER JOIN TRAVAIL ON IDSERV=IDSERVICE INNER JOIN VOISIN ON IDVOISIN=IDVOIS);
...
SELECT NOM,PRENOM FROM VOISIN WHERE IDVOISIN = (SELECT IDVOIS FROM TRAVAIL GROUP BY IDVOIS HAVING COUNT(*) >= (SELECT MAX(COUNT(*)) FROM  TRAVAIL GROUP BY IDVOIS));
